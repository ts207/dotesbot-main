...
def run_compare():
    try:
        ...
        # Drop rows with NaN markouts to ensure a fair comparison
        combat = combat.dropna(subset=['markout_3s', 'markout_10s', 'markout_30s'])
        
        print(f"Comparing stop losses on {len(combat)} historical combat signals:")
        for stop_val in [None, 0.01, 0.03, 0.05, 0.10]:
            ...
            for _, row in combat.iterrows():
                ...
                if stop_val is not None:
                    ...
                    # (Simplified: if price gapped past stop, we take the gap price)
