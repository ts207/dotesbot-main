...
class PaperTrader:
    ...
    def _position_from_trade_row(self, row: dict[str, str]) -> Position | None:
        try:
            ...
            return Position(
                ...
                target_horizon=row.get("target_horizon") or None,
                expected_hold_sec=self._optional_int(row.get("expected_hold_sec")),
                entry_trigger=row.get("entry_trigger") or None,
                exit_trigger=row.get("exit_trigger") or None,
                primary_metric=row.get("primary_metric") or None,
                secondary_metric=row.get("secondary_metric") or None,
    ...
    def enter(
        ...
        pos = Position(
            ...
            target_horizon=signal.get("target_horizon"),
            expected_hold_sec=self._optional_int(signal.get("expected_hold_sec")),
            entry_trigger=signal.get("entry_trigger"),
            exit_trigger=signal.get("exit_trigger"),
            primary_metric=signal.get("primary_metric"),
            secondary_metric=signal.get("secondary_metric"),
            promotion_rule=signal.get("promotion_rule"),
            disable_rule=signal.get("disable_rule"),
    ...
    def _close_position(
        ...
        cp = ClosedPosition(
            ...
            target_horizon=pos.target_horizon,
            expected_hold_sec=pos.expected_hold_sec,
            entry_trigger=pos.entry_trigger,
            exit_trigger=pos.exit_trigger,
            primary_metric=pos.primary_metric,
            secondary_metric=pos.secondary_metric,
            promotion_rule=pos.promotion_rule,
            disable_rule=pos.disable_rule,
