...
def main() -> int:
    ...
    print("\n3. Historical data:")
    require_data_v2 = os.getenv("REQUIRE_DATA_V2_PREFLIGHT", "false").lower() in {"1", "true", "yes"}
    ...
    for table in ["snapshots", "book_ticks"]:
        ...
        ok = files > 0 or not require_data_v2
        detail = f"{len(partitions)} date partitions, {files} parquet files"
        if files == 0 and not require_data_v2:
            detail += " (optional for runtime; set REQUIRE_DATA_V2_PREFLIGHT=true to require)"
        all_ok &= _check(f"data_v2/{table}", ok, detail)
