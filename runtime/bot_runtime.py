...
def _audit_and_quarantine_mappings(
    ...
    *,
    ...
    issues = audit_mappings(
        ...
        books_by_yes_token=books_by_yes_token,
    )
    quarantined = quarantine_critical_issues(markets, issues)
    try:
        from state_store import StateStore
        StateStore().record_mapping_snapshots(markets)
    except Exception as exc:
        print(f"mapping snapshot mirror error (non-fatal): {exc}")
    if quarantined:
        ...
        while True:
            try:
                ...
                if now - last_mapping_refresh >= MAPPING_REFRESH_SECONDS:
                    ...
                    books_by_yes_token = {
                        str(m.get("yes_token_id")): book_store.get(str(m.get("yes_token_id")))
                        for m in raw_markets
                        if m.get("yes_token_id") and book_store.get(str(m.get("yes_token_id")))
                    }
                    quarantined = _audit_and_quarantine_mappings(
                        raw_markets,
                        games_for_sync,
                        books_by_yes_token=books_by_yes_token,
                    )
                    if updates or quarantined:
                        write_markets(mdata)
                    if updates:
                        for u in updates:
                            print(
                ...
                closed = trader.check_exits(
                    ...
                    current_games_by_match_id=last_steam_games,
                )
                            ...
                            # ── DSWING ─────────────────────────────────────────────────────
                                ...
                                if live_executor is not None and live_position_store is not None:
                                    ...
                                    ds_signal_dict = {
                                        ...
                                        "entry_is_reversal": False,
                                        "entry_is_continuation": False,
                                        "would_pass_live_gates": ds_res.would_pass_live_gates,
                                        "would_pass_live": ds_res.would_pass_live,
                                        "live_skip_reason": ds_res.live_skip_reason,
                                        "paper_only_bypass": ds_res.paper_only_bypass,
                                        "policy_allowed": ds_res.policy_allowed,
                                        "policy_reason": ds_res.policy_reason,
                                        "policy_version": ds_res.policy_version,
...
async def main():
    ...
    try:
        ...
        quarantined = _audit_and_quarantine_mappings(raw_markets, games)
        if updates or quarantined:
            write_markets(mdata)
        if updates:
            for u in updates:
    ...
    def _on_book_update():
        ...
        for cp in trader.check_exits(
            store,
            set(),
            None,
            current_games_by_match_id=_last_steam_games,
        ):
            position_logger.log_exit(cp)
            print(
                f"EXIT [{cp.exit_reason}] {cp.market_name} {cp.side} "
...
class BotRuntime:
    """Thin object wrapper for the legacy async bot runtime.

    The first refactor step moves orchestration out of main.py without changing
    the trading loop. Follow-up runtime modules can carve this file into feed,
    mapping, strategy, execution, and markout components behind this interface.
    """
